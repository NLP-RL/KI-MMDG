...
Original file is located at
    https://colab.research.google.com/drive/1M9MKnrd86YNelzMzA89wrSzXVvVv49tq
"""

! pip -q install transformers==2.9.0 gdown
! pip instal  l ipywidgets
...
lens, tok_cnt, word_cnt = get_counter_and_lens(trn_df[trn_df.columns].apply(lambda x: ' '.join(x.astype(str)), axis = 1), tokenizer)
...
for step in tqdm(range(len(df))):
    # encode the new user input, add the eos_token and return a tensor in Pytorch
    new_user_input_ids = tokenizer.encode(input[step] + tokenizer.eos_token, return_tensors='pt')
    ...
    predicted.append(format(tokenizer.decode(chat_history_ids[:, bot_input_ids.shape[-1]:][0], skip_special_tokens=True)))

original=list(val_df['output'])

df = pd.DataFrame(list(zip(val_df['input'], predicted, original)),
               columns =['input', 'predicted', 'real'])
...
"""# Saving the generated outputs in a csv file"""

df = pd.DataFrame(list(zip(input, predicted, original)),
               columns =['input','predicted', 'real'])
